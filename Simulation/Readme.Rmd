...
```{r 2d FN MLE simulation, echo=TRUE, warning=FALSE}
set.seed(123456)
simu_n <- 1000
dim_p <- 2
...
```{r 4d FN MLE simulation, echo=TRUE, warning=FALSE}
set.seed(123456)
simu_n <- 1000
dim_p <- 4
result_df <- data.frame()
...
```{r real_data, eval=FALSE,warning=FALSE}
source("utils.R")
library(MASS)
library(ggplot2)
library("shape")
